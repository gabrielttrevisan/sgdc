...
export const Beneficiaries = () => {
  /** @type {[import("./BeneficiariesService").Beneficiary[], import("react").SetStateAction<import("./BeneficiariesService").Beneficiary[]>]} */
  const [page, setPage] = useState([]);

  useEffect(() => {
    BeneficiariesService.list().then((response) => {
      console.log(response);
      setPage(response.data?.items ?? []);
    });
  }, []);

  if (!page.length) return <div>Nada encontrado</div>;
