    ...
    <div className="container">
      <main className="content">
        <div className="page-card">
          ...
          {deleteMessage && (
            <div
              className={`notice ${deleteMessage === "Exclusão cancelada." ? "cancel" : ""}`}
            >
              {deleteMessage}
            </div>
          )}
          ...
          <div className="table-footer">
            <span>
              Exibindo {filteredDonors.length === 0 ? 0 : startIndex + 1} -{" "}
              {Math.min(endIndex, filteredDonors.length)} de{" "}
              {filteredDonors.length} doadores
            </span>
            <Pagination
